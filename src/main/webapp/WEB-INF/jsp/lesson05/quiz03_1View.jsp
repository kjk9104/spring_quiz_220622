		...
		<table class="table text-center">
			...
			<c:forEach var="candidate" items="${candidates}" varStatus="status">
				<tbody>
					<tr>
						<td>${status.count}</td>
						<td><fmt:formatNumber value="${candidates[status.index]}"/></td>
						<td><fmt:formatNumber type="percent" value="${candidates[status.index]/1000000}"/></td>
					</tr>
				</tbody>
			</c:forEach>
		</table>
			...
			<c:forEach var="cardBill" items="${cardBills}" varStatus="status">
				<tbody>
					<tr>
						...
						<fmt:parseDate value = "${cardBill.date}"
						    pattern = "yyyy-MM-dd" var = "date"/>
						<td><fmt:formatDate value="${date}" pattern="yyyy년 MM월 dd일"/>
						</td>
